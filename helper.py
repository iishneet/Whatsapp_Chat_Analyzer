...
def sentiment_analysis(df, selected_user):
    ...
    return df
def sentiment_summary(selected_user, sentiment_df):
    if selected_user != 'Overall':
        sentiment_df = sentiment_df[sentiment_df['user'] == selected_user]

    # Group by sentiment label to count each category
    sentiment_summary = sentiment_df['sentiment_label'].value_counts().reset_index()
